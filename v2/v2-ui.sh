...
install_v2ray() {
    ...
    cd /root
    wget https://flashproxy.cn/v2/go.sh
    bash go.sh
    if [[ $? -ne 0 ]]; then
...
install_v2-ui() {
    ...
    #wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz http://yuan.jiyunidc.com/v2/ui/${last_version}/v2-ui-linux.tar.gz
    wget -N --no-check-certificate -O /usr/local/v2-ui-linux.tar.gz https://flashproxy.cn/v2/v2-ui-linux.tar.gz
    if [[ $? -ne 0 ]]; then
    ...
    echo -e ""
    curl -o /usr/bin/v2-ui -Ls https://flashproxy.cn/v2/v2-ui.sh
