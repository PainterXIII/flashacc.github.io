...
+ function($) {
    ...
    //修改密码
    $.setpass = function(text, title, onOK, onCancel, username, password,pass2) {
        var config;
        if (typeof text === 'object') {
            config = text;
        } else {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
                '<input type="password" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="当前密码" />' +
                '<input type="password" class="weui-input weui-prompt-input" id="weui-prompt-password" value="' + (config.password || '') + '" placeholder="输入新密码" />' +
                '<input type="password" class="weui-input weui-prompt-input" id="weui-prompt-password2" value="' + (config.pass2 || '') + '" placeholder="再次输入" />',
            title: config.title,
            autoClose: false,
            buttons: [
                {
                    ...
                    onClick: function() {
                        var username = $("#weui-prompt-username").val();
                        var password = $("#weui-prompt-password").val();
                        var password2 = $("#weui-prompt-password2").val();
                        if (!config.empty && (username === "" || username === null)) {
                            $.toast("当前密码不能为空",'text');
                            modal.find('#weui-prompt-username').focus()[0].select();
                            return false;
                        ...
                        if(openvpn.pass != username){
                            $.toast("当前密码错误",'text');
                            modal.find('#weui-prompt-username').focus()[0].select();
                            return false;
                        }
                        if(password2 != password){
                            console.log('password2:'+password2+'password:'+password)
                            $.toast("两次新密码不一致",'text');
                            modal.find('#weui-prompt-password').focus()[0].select();
                            return false;
                        }
                        if(strlen(password) > 30 || strlen(password) < 6){
                            $.toast("密码不能小于6位或大于30位",'text');
                            modal.find('#weui-prompt-password').focus()[0].select();
                            return false;
                        }

                        if(funcChina(password) == false){
                            $.toast("密码不能含有中文",'text');
                            modal.find('#weui-prompt-password').focus()[0].select();
                            return false;
                        }
            ...
            '<div class="weui-cells weui-cells_radio">\
              ...
              <label class="weui-cell weui-check_label" for="weui-select-id-{{this.title}}">\
                ...
                <div class="weui-cell__ft">\
                  ...
                  <span class="weui-icon-checked"></span> 选择\
                </div>\
              </label>\
              {{/items}}\
            </div>',
